...
# Definir los puntos existentes y los puntos adicionales
...
# Función para calcular la varianza de estimación
calcular_varianza <- function(existing_wells, selected_points) {
...
# Implementación del método de intercambios secuenciales óptimos
n <- nrow(additional_points)  # Número total de puntos
k <- 4  # Número de puntos a usar
mejores_varianzas <- numeric(0)
mejores_combinaciones <- list()

j <- 1

while (j <= n - k) {
  for (i in 1:k) {
    combinaciones <- t(combn(nrow(additional_points), k))
    puntos_seleccionados <- additional_points[combinaciones[i, ], ]
    varianza <- calcular_varianza(existing_wells, rbind(existing_wells, puntos_seleccionados))
    if (!(varianza %in% mejores_varianzas)) {
      mejores_varianzas <- c(mejores_varianzas, varianza)
      mejores_combinaciones[[length(mejores_combinaciones) + 1]] <- puntos_seleccionados
    }
  }
  j <- j + 1
}
...
orden <- order(mejores_varianzas)
mejores_varianzas <- mejores_varianzas[orden]
mejores_combinaciones <- mejores_combinaciones[orden]

# Mostrar las 10 mejores combinaciones y sus varianzas
resultados <- data.frame(varianza = mejores_varianzas[1:3], combinacion = sapply(mejores_combinaciones[1:3], function(x) paste(rownames(x), collapse = ",")))
print(resultados)

# Crear un data frame con los puntos seleccionados y sus identificadores
mejor_combinacion <- mejores_combinaciones[[1]]
mejor_combinacion$ID <- paste0("P", seq_len(nrow(mejor_combinacion)))

# Graficar los pozos existentes, los puntos adicionales y las estaciones seleccionadas
ggplot() +
  geom_polygon(data = block_V, aes(x = x, y = y), fill = "lightblue", alpha = 0.5) +
  geom_point(data = existing_wells, aes(x = x, y = y, color = "Pozos Existentes"), size = 3) +
  geom_point(data = additional_points, aes(x = x, y = y, color = "Puntos no escogidos"), size = 3) +
  geom_point(data = mejor_combinacion, aes(x = x, y = y, color = "Puntos escogidos"), size = 3) +
  geom_text(data = mejor_combinacion, aes(x = x, y = y, label = ID), vjust = -0.5) +  # Agregar etiquetas de los puntos
  scale_color_manual(values = c("Pozos Existentes" = "red", 
                                "Puntos no escogidos" = "blue", 
                                "Puntos escogidos" = "green"),
                     name = "Leyenda") +
  ggtitle("Distribución de Pozos Existentes y Puntos Adicionales") +
  xlab("Longitud") +
  ylab("Latitud") +
  theme_minimal()

print(resultados$varianza/50000)
